...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated source HEAD `a245f24`; repository validation clean; fresh Photon-import compile path succeeds; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest fixed-tick continuation validation passes 89/89 EditMode and 37/37 PlayMode tests; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Fresh fixed-tick development IL2CPP APK (`151,318,469` bytes; SHA-256 `154F0F81FA6F068302FC57A6513A6C38CB4A7A8298C856264A04B14EE796574F`) installed/launched on Lava `ST5GDW23LB004392`; the prior monotonic-tick Unity error no longer reproduced in the current process; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Web smoke build | Passed with evidence | Fresh fixed-tick Web build (`21` build files; `133,187,266` bytes; main WASM `120,495,480` bytes; SHA-256 `F02758353612C68904777C2F25F4F56E22DA4225F0F426A205792BE41E111EC6`) served locally on port 8137 with HTTP 200 and inspected live-match screenshot; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Explicit fixed simulation clock | Passed with evidence | 30 Hz clock now exposes the exact tick for every step consumed in a render frame; authority, movement, attacks, projectiles, bots, gadgets and all three fighter adapters use per-step identities. 89 EditMode/37 PlayMode tests, fixed Android Lava retest and Web smoke evidence pass; replay recording and broader soak coverage remain |
