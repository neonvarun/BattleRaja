...
Latest continuation: `d1b33d1` + `4d3ae6a` + `ff2a3e4` + `8645254` + `044b1b8` + `2809165` + `a170746` + `08c6f2e` + `678acb0` (2026-08-03). Production-flow
...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated runtime source HEAD `678acb0`; Photon Fusion `2.1.1 Stable 2177`; Input System-only handler with legacy-scene compatibility bridge; repository validation clean; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest authority-first health regression passes 97/97 EditMode and 48/48 PlayMode tests, including canonical actor damage/healing, duplicate prevention, readable touch labels, results/rematch and Maya coverage; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Current `678acb0` APK (`151,415,947` bytes; SHA-256 `4B1BAF633BA448555B5D3E6C4C21666C61462499D0C3451CFE596AEA1234CFD3`) installed/launched only on Lava `ST5GDW23LB004392`; portrait controls remain visible. The optional Play Asset Delivery class-probe warning remains known. |
| Web smoke build | Passed with evidence | Current `678acb0` Web output (`21` files; `133,374,016` bytes; `Web.wasm` SHA-256 `1DBDC8DA65514A660DC4651EF7116FB83C522C42F62E0678716B059F7B76438E`) built successfully and returned HTTP 200 locally; browser visual/input and human review gates remain open; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Authoritative rule separation | In progress | Actor damage and pickup/Tiffin healing now resolve in `OfflineMatchAuthority` against canonical participant health/statistics before Unity applies snapshots/events. Zone damage, pickup availability/respawn, deterministic collection, gadget runtime rules, Dhol displacement, Tiffin lifetime/damage/destruction, Umbrella mitigation and results remain application-owned; movement reconciliation, remaining presentation adapters and network transport remain |
