...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated source HEAD `67a4d40`; repository validation clean; fresh Photon-import compile path succeeds; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest responsive-framing validation passes 87/87 EditMode and 36/36 PlayMode tests; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Fresh responsive-framing development IL2CPP APK (`151,288,356` bytes; SHA-256 `1F9A7E6DAF65AE963E167FE8D055AE8C94757C4A2E9EE1ECE8B2BCDCA24BEA9F`) built; Lava-only physical install remains unavailable because the Lava serial is not currently connected; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Web smoke build | Passed with evidence | Fresh responsive Web build (`21` build files; `133,184,187` bytes; main WASM `120,493,105` bytes; SHA-256 `027599E8E1A201157C86305DA775A8F500048E89E9318FA002DE812E68186E74`) served locally on port 8137 with HTTP 200; the production Bootstrap → Bazaar route also rebuilt and served on port 8138, with responsive portrait gameplay/tutorial captures; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Visual and interaction QA | In progress | `Docs/QA/VISUAL_QA_REPORT.md` records Playwright evidence at 1920×1080, 1440×900, 1280×720, 1024×768 and 390×844. Responsive portrait framing now fills the tested viewport; dense HUD, gadget use, a distinct loading surface, results/rematch and final human approval remain unverified |
