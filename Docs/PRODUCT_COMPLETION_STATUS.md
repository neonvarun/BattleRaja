...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated source HEAD `ac60062` (runtime-bearing candidate `4391f09`); repository validation clean; fresh Photon-import compile path succeeds; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest authority/tick validation passes 83/83 EditMode and 33/33 PlayMode tests; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Authoritative rule separation | In progress | Zone damage, pickup availability/respawn, gadget collection/inventory/use cooldown, elimination, placement and results are application-owned; authoritative gadget cooldowns now advance per tick and damage intents carry tick identity. Dhol/Tiffin effect execution and Unity presentation adapters remain |
