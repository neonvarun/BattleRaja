...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated source HEAD `d328132` (runtime-bearing candidate `4391f09`); repository validation clean; fresh Photon-import compile path succeeds; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest Umbrella-authority validation passes 86/86 EditMode and 34/34 PlayMode tests; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Fresh authority candidate `d328132` development IL2CPP APK (`151,286,644` bytes; SHA-256 `B50F74E56A3F3FA95A164834223739B3A0A2F1681DBA3D12F7B82B956F682F9B`) built; Lava-only physical install remains unavailable because the Lava serial is not currently connected; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Web smoke build | Passed with evidence | Fresh authority candidate `d328132` Web build (`21` build files; `133,177,869` bytes; main WASM SHA-256 `6538B2ECF83940FEB7F9AE7F1B160114557E04F2E49F847048AD9795D711EB9E`) served locally on port 8136 with HTTP 200; browser bootstrap evidence remains from the prior Web smoke; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Authoritative rule separation | In progress | Zone damage, pickup availability/respawn, deterministic pickup/gadget proximity selection, gadget collection/inventory/use cooldown, Dhol displacement, Tiffin healing/lifetime and Umbrella mitigation are application-owned; Unity consumes immutable collection/damage/displacement/healing intents. Station damage forwarding and remaining Unity presentation adapters remain |
