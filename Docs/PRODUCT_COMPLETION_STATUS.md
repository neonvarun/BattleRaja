...
Latest continuation: `efbb1ac` + `8f190cd` + `26b11cc` + `d993a5b` + `9100e69` + `78fa990` + `204e4f0` + `678acb0` (2026-08-03).
...
| Android smoke build | Passed with evidence | Fresh current-HEAD prefab APK (`151,511,951` bytes; SHA-256 `4837EACB14E2E5166E85AD034F4C88F453AC260AF695BE2FFEBB7A8A8AE7A14A`) installed/launched only on Lava `ST5GDW23LB004392` (`LAVA LXX508`); Unity activity remained top-resumed with no fatal exception/SIGSEGV/app-process crash marker. Memory snapshot: 408,271 KB PSS / 545,328 KB RSS / 82,088 KB Graphics. The optional Play Asset Delivery class-probe warning remains known. |
| Web smoke build | Passed with evidence | Fresh current-HEAD Web output (`19` files; `133,496,106` bytes; `Web-BazaarBastion.wasm` SHA-256 `C1375376EF0906364B33798E046F719C041ECB1BF0277DBFCA32B91ABA165C68`) built successfully and returned HTTP 200 locally on port 8139; Playwright visual/interaction captures cover the primary flow and results/rematch, while gadget use, multi-browser and human review remain open. |
...
| Performance, soak, multi-browser, and release gates | In progress | Current Lava snapshot records 408,271 KB PSS, 545,328 KB RSS and 82,088 KB Graphics; device log frame samples are around 30 FPS during the active Web/Android smoke route, but frame-time/FPS/GPU/GC, repeated-match growth, thermal/battery and multi-browser release measurements remain open; see `Docs/PERFORMANCE_BUDGET.md` and `Docs/QA/LATEST_HEAD_BASELINE.md` |
