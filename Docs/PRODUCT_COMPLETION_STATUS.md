...
Latest continuation: `9100e69` + `78fa990` + `204e4f0` + `678acb0` (2026-08-03). Production-flow
...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated runtime source HEAD `9100e69`; Photon Fusion `2.1.1 Stable 2177`; Input System-only handler with legacy-scene compatibility bridge; repository validation clean; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest authority-fighter-displacement regression passes 99/99 EditMode and 50/50 PlayMode tests, including canonical actor movement, damage/healing, duplicate prevention, readable touch labels, results/rematch, Maya coverage and live Bijli displacement; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Current `9100e69` APK (`151,465,096` bytes; SHA-256 `00D83717FAC958A588B4F4964504C92ACA70CDE23DFE800C99234CC48A6ED3E8`) installed/launched only on Lava `ST5GDW23LB004392` (`LAVA LXX508`); Unity activity remained top-resumed with no sampled fatal marker. The optional Play Asset Delivery class-probe warning remains known. |
| Web smoke build | Passed with evidence | Current `9100e69` Web output (`21` files; `133,477,944` bytes; `Web.wasm` SHA-256 `F6C37FFC4D3CAE8DE87BDFAF5202FE32F7FF504AE66BB4C55D7834C50852D6EB`) built successfully and returned HTTP 200 locally; browser visual/input and human review gates remain open; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Authoritative rule separation | In progress | Actor damage, pickup/Tiffin healing, production movement, Dhol displacement and Bijli/Pehel ability displacement now resolve through `OfflineMatchAuthority` against canonical participant state before Unity applies snapshots/events. Bazaar movement rejects duplicate ticks and disables the local CharacterController projection; MovementLab remains an observation fixture. Fighter runtime ownership, remaining presentation adapters and network transport remain open |
