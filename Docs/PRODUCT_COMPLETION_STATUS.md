...
Latest continuation: `204e4f0` + `678acb0` (2026-08-03). Production-flow
...
| --- | --- | --- |
| Unity project and package baseline | Passed with evidence | Unity `6000.5.6f1`; latest validated runtime source HEAD `204e4f0`; Photon Fusion `2.1.1 Stable 2177`; Input System-only handler with legacy-scene compatibility bridge; repository validation clean; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| EditMode and PlayMode regression baseline | Passed with evidence | Latest authority-movement regression passes 98/98 EditMode and 49/49 PlayMode tests, including canonical actor movement, damage/healing, duplicate prevention, readable touch labels, results/rematch and Maya coverage; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
| Android smoke build | Passed with evidence | Current `204e4f0` APK (`151,468,901` bytes; SHA-256 `378B0474577A575139FEE1F797EF848C2FC27335CCA69427F64297860A768A04`) installed/launched only on Lava `ST5GDW23LB004392` (`LAVA LXX508`); Unity activity remained top-resumed with no sampled fatal marker. The optional Play Asset Delivery class-probe warning remains known. |
| Web smoke build | Passed with evidence | Current `204e4f0` Web output (`21` files; `133,467,594` bytes; `Web.wasm` SHA-256 `DFA4599679634349459DBD68973467B929F9470D7FAC057A56B90D5C137C46FC`) built successfully and returned HTTP 200 locally; browser visual/input and human review gates remain open; see `Docs/QA/LATEST_HEAD_BASELINE.md` |
...
| Authoritative rule separation | In progress | Actor damage, pickup/Tiffin healing and production movement now resolve in `OfflineMatchAuthority` against canonical participant state before Unity applies snapshots/events. Bazaar movement rejects duplicate ticks and disables the local CharacterController projection; MovementLab remains an observation fixture. Zone damage, gadget displacement/ability movement, remaining presentation adapters and network transport remain open |
