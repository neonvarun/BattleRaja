...
`getconf PAGESIZE` returned **16384**, and the ABI list includes `x86_64,arm64-v8a`. A clean
direct launch rendered the branded menu and live-match checkpoints normally. The final menu
capture is `Builds/Local/Device/Performance/20260830-16k-5d136fb/host-gpu/launch-final.png`
(SHA-256 `919BA18BBCA77C4C843DD07EC1470E8D0DFAE4AC3C3F012266E102ACABD55FA0`); the app-scoped
launch logcat has no configured fatal, ANR, SIGSEGV, SIGABRT or shader-link marker.
...
**31,416 KB**, and process `top` CPU **96.1-123.0%** on Android's 100%-per-core scale. The
virtual battery stayed at 100%/5,000 mV/25 C and thermal status 0. Unity `gfxinfo` has no
usable frame histogram; this is synthetic runtime/page-size evidence, not a product-tier
performance pass. SwiftShader on the same AVD produced URP/Lit uniform-limit corruption and
is retained as a superseded renderer diagnostic. Host-GPU 16 KB smoke is passed locally;
physical 16 KB, other GPU profiles, normalized budgets and human approval remain open.

## P50 exact-candidate Lava live-match SurfaceFlinger diagnostic - 2026-08-30

The exact `5d136fb` APK was relaunched on approved Lava `ST5GDW23LB004392` through Rematch.
During approximately 45 seconds of a live Solo Raja match, SurfaceFlinger reported **126
valid present timestamps** and **125 intervals** after excluding one `Long.MaxValue`
sentinel. Using the middle timestamp column, min/median/p95/p99/max intervals were
**16.447 / 16.534 / 16.565 / 33.078 / 33.367 ms**; three intervals exceeded one refresh
period and one exceeded 2×. The raw folder is
`Builds/Local/Device/Performance/20260830-lava-5d136fb-sf/`; its summary SHA-256 is
`21369E4FC3BF33BF1DB234BE2F23F1A8D32BD45D0DF29F8682DC90D17489B144`, and the latency-file
SHA-256 is `D83D61790C60E5D76CB9BBC5B0D25CA91D0AD044BC63686DAD417F71942B3D26`.

The end screenshot shows player defeat and spectator state while Aandhi closes. End
telemetry was **277,284 KB PSS / 400,500 KB RSS / 80,052 KB graphics PSS**, battery
...
performance and Play Console gates remain open.
...
9101. Runtime/art source remains `5d136fb`; PlayMode passed **89/89**. All **100/100**
matches reached terminal results at **306.0135193 s** each and landed in the 240-360 second
window. Every match had combat and bot-to-bot damage, **0/100** were Aandhi-only, protected-
warmup and invalid-position samples were zero, and Umbrella Guard, Dhol Burst and Tiffin
Station were each used in all 100 matches. Exact artifact paths and SHA-256 values are
indexed in `Docs/V1_RELEASE_PLAN.md` P53.

This closes the current automated pacing/safety gate locally. The accelerated 50x path is
not same-seed command-stream determinism evidence; P10's real-time result remains the
determinism record. Human route/accessibility/fun, authored/cultural, normalized performance,
