...
## Latest exact-current release-shaped slice — `7b13f8f` — 2026-08-24

The exact current source is `7b13f8f` on `codex/v1-playstore-release`. Validation is
...
`C:\Users\USER\AppData\Local\Temp\battleraja-lava-7b13f8f\release-menu.png`.
It shows the player-facing `OFFLINE ARCADE • 8-ACTOR MATCHES` footer and no
development-build watermark. The Unity surface exposes no actionable nodes in the
Android UI tree, so this capture does not prove touch navigation beyond launch/menu.

The exact Web build is 19 files / **132,071,670 bytes**, with a
119,799,945-byte WASM (`53354CCE0B394FD0CEC7F9C3553B6BF8CDBB0E18A1F96F03274BA38104B6EECF`).
Chrome and Edge loader screenshots are in
`C:\Users\USER\AppData\Local\Temp\battleraja-web-7b13f8f\`; both stayed on the
